...
def post(path, body, tries=4):
    ...
    for a in range(tries):
        ...
        try:
            ...
            if "Duplicate" in msg: return 409, msg
            time.sleep(1+a)
        except Exception:
            time.sleep(1.5+a)
...
out=subprocess.check_output(["docker","exec","citizens-postgres","psql","-U","citizens","-d","citizens","-tAc",sql]).decode()
rows=[json.loads(l) for l in out.splitlines() if l.strip()]
print("citizens read:",len(rows),flush=True)
ok=dup=fail=0
for i,c in enumerate(rows):
    p={"nationalId":c["national_id"],"pin":"111111","password":"111111",
