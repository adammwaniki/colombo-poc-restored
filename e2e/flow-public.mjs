...
const base="http://156.67.105.185:8080", NID="80000002", PIN="100001";
...
await wait(7000); await shot("ep-1-esignet"); console.log("URL@esignet:",p.url().split("?")[0]);
await p.evaluate(()=>document.querySelector("#login_with_pin")?.click()); await wait(4000);
await reactSet("#Pin_mosip-uin",NID); await reactSet("#Pin_pin",PIN); await wait(1000); await shot("ep-2-pinform");
console.log("submit:",await clickText("button","Login"));
await wait(9000); await shot("ep-3-consent"); console.log("URL after pin:",p.url().split("?")[0]);
...
await wait(9000); await shot("ep-4-final");
