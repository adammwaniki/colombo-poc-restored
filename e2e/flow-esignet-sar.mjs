...
const base="http://156.67.105.185:8080", NID="80000006", PIN="100005";
...
await wait(7000); await shot("sar-1-esignet"); console.log("URL@esignet:",p.url().split("?")[0]);
await p.evaluate(()=>document.querySelector("#login_with_pin")?.click()); await wait(4000);
await reactSet("#Pin_mosip-uin",NID); await reactSet("#Pin_pin",PIN); await wait(1000); await shot("sar-2-pinform");
console.log("submit:",await clickText("button","Login"));
await wait(9000); await shot("sar-3-consent"); console.log("URL after pin:",p.url().split("?")[0]);
...
await wait(9000); await shot("sar-4-final");
console.log("FINAL URL:",p.url().split("#")[0].slice(0,120));
console.log("BODY:",(await p.evaluate(()=>document.body.innerText)).replace(/\s+/g," ").slice(0,260));
await b.close();
